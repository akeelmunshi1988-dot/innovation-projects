  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {loading ? <p className="text-dark-400">Loading display images…</p> : loadedCategory !== category ? <p className="text-dark-400">Reload the page to retry loading this category.</p> : <>
        ...
        {display.images.every(image => image.image_url) && <div><h2 className="mb-3 text-sm text-cream-300">Preview</h2><CollectionImageGrid state={{ status: 'ready', display }} title={category === 'default' ? 'The Rug Collection' : `${selected?.label.split(': ')[1] || category} Rugs`} eyebrow="Collection preview" target={selected?.href || '/catalog'} /></div>}
      </>}
    </div>
  );
}
