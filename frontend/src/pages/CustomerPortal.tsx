...
export default function CustomerPortal() {
  ...
  useEffect(() => {
    ...
    drawCanvas(points);
  }, [roomPreview, points, selectedRug]);
  ...
  useEffect(() => {
    ...
    if (imageDims.w > 0 && pendingRugRef.current && file && !customMode) {
  ...
  const drawCanvas = (pts: Point[]) => {
    ...
    img.onload = () => {
      ...
      pts.forEach(([x, y], i) => {
        ...
        ctx.fillStyle = ["#f97316", "#eab308", "#22c55e", "#3b82f6"][i]; ctx.fill();
        ctx.fillStyle = "#fff";
      ...
      if (pts.length === 4) {
        ...
        ctx.setLineDash([Math.max(8, canvas.width / 80), Math.max(5, canvas.width / 120)]);
        ctx.stroke();
        ctx.setLineDash([]);
  ...
  const handleSelectRug = (rug: CatalogRug) => {
    setSelectedRug(rug);
    setResultImage("");
    setError("");
    setCustomMode(false);
    ...
    if (imageDims.w > 0 && file) {
      ...
      setPoints([]);
      pendingRugRef.current = rug; // fires when imageDims are ready
    }
  ...
  const handleCanvasClick = (e: React.MouseEvent<HTMLCanvasElement>) => {
    if (!customMode || points.length >= 4) return;
    const canvas = canvasRef.current!;
    const rect = canvas.getBoundingClientRect();
    const scaleX = canvas.width / rect.width;
    const scaleY = canvas.height / rect.height;
    setPoints((prev) => [...prev, [(e.clientX - rect.left) * scaleX, (e.clientY - rect.top) * scaleY]]);
  };
  ...
  const generateRug = async () => {
    if (!roomFile || !selectedRug || points.length !== 4) return;
  ...
  const handlePlaceOrder = async () => {
    ...
    if (!w || !h) {
      navigate(`/shop/catalog/${selectedRug.id}`);
      return;
    }
    try {
            ...
            {resultImage ? (
              ...
              <>
                <div className="flex items-center justify-between gap-3 flex-wrap">
                  ...
                  <div className="flex gap-2">
                    ...
                    >
                      <ShoppingBag size={11} /> Place Order
                    </button>
                  </div>
                </div>
              ...
              <>
                    ...
                    {loading ? (
                      ...
                      <>
                        <h2 className="text-stone-900 font-medium text-sm">
                          {points.length < 4
                            ? `Click corner ${points.length + 1} of 4 on the floor`
                            : "4 corners set — click Generate"}
                        </h2>
                        <p className="text-stone-400 text-xs mt-0.5">top-left → top-right → bottom-right → bottom-left</p>
                      </>
                    ) : (
                ...
                <div className="relative overflow-hidden border border-stone-200">
                  <canvas ref={canvasRef} onClick={handleCanvasClick} className="w-full block"
                    style={{ cursor: customMode && points.length < 4 ? "crosshair" : "default" }}
                  />
                  {loading && (
                  ...
                  {quoteSubmitted && quoteResult ? (
                    <div className="p-8 flex flex-col items-center gap-4 text-center">
                      ...
                      <div className="flex gap-10 border-t border-stone-100 pt-4 w-full justify-center">
                        ...
                        <div className="text-center">
                          <p className="text-stone-400 text-xs uppercase tracking-widest mb-1">Lead Time</p>
                      ...
                      <div className="space-y-5">
                        ...
                        <div className="flex items-center gap-3 border border-stone-100 bg-stone-50 p-3">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="text-stone-600 text-xs mt-0.5">{sym}{selectedRug.base_price_per_sqm}/sqm · {selectedRug.lead_time_days}d lead time</p>
                          </div>
                        </div>
